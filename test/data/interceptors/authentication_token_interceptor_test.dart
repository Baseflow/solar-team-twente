...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(Token(accessToken: 'accessToken', refreshToken: 'refreshToken', expiresAt: DateTime.now()));
  ...
  group(
    ...
    () => <void>{
      test('Should add token to request', () async {
        ...
        await tokenInterceptor.onRequest(requestOptions, RequestInterceptorHandler());

        expect(requestOptions.headers['Authorization'], 'Bearer accessToken');
      }),
      test('Should not add token if no token is available', () async {
        when(() => mockAuthenticationRepository.getToken()).thenAnswer((_) => Future<Token?>.value());

        final RequestOptions requestOptions = RequestOptions(path: '/');

        expect(
          () async => tokenInterceptor.onRequest(requestOptions, RequestInterceptorHandler()),
          throwsA(const TokenException(errorCode: TokenExceptionCode.noTokenFound)),
        );
      }),
      test('Should refresh token and retries request on 401 error', () async {
        ...
        when(() => mockAuthenticationRepository.getToken()).thenAnswer((_) => Future<Token?>.value(initialToken));
        when(
        ...
        when(() => mockDio.fetch<dynamic>(any())).thenAnswer(
          (_) => Future<Response<dynamic>>.value(
            Response<dynamic>(statusCode: 200, requestOptions: RequestOptions(path: '/some-path')),
          ),
        );
        ...
        verify(() => mockAuthenticationRepository.refreshToken(initialToken)).called(1);
      ...
      test('Should throw an exception when refresh fails', () async {
        ...
        when(() => mockAuthenticationRepository.getToken()).thenAnswer((_) => Future<Token?>.value(token));
        when(
          () => mockAuthenticationRepository.refreshToken(token),
        ).thenThrow(const TokenException(errorCode: TokenExceptionCode.localSaveFailed));

        final RequestOptions requestOptions = RequestOptions(path: 'https://example.com');
      ...
      test('Should forward exceptions when a non-401 exception is thrown', () async {
        when(() => mockAuthenticationRepository.getToken()).thenAnswer(
          (_) => Future<Token?>.value(
            Token(
              accessToken: 'accessToken',
        ...
        when(() => mockErrorInterceptorHandler.next(dioException)).thenThrow(dioException);

        expect(() => tokenInterceptor.onError(dioException, mockErrorInterceptorHandler), throwsA(dioException));
      }),
    },
  );
}
